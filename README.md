...
просыпается каждые 5 минут (поэтому ответ приходит за несколько минут, а не мгновенно), забирает новые
сообщения, отвечает на команды и рассылает прогноз тем пользователям, у кого в часовом поясе их города
наступил час рассылки (по умолчанию **18:00 по местному времени города**).
...
задержаться на 5–20 минут. Поэтому бот отвечает не мгновенно, а в течение нескольких минут: он не держит
постоянного соединения с Telegram, а забирает накопившиеся сообщения по расписанию. Прогноз всё равно придёт
один раз, потому что дата последней отправки хранится в `state.json`.

Если ответа нет дольше 15 минут, откройте `Actions → Weather bot`: последний запуск должен быть зелёным,
а в его логе должны быть строки «Обработано сообщений: N» (или «Новых сообщений нет»). Быстрый способ
получить ответ немедленно — `Run workflow` без параметров (см. ниже). Если репозиторий не проявляет
активности 60 дней, GitHub отключает расписание — тогда включите workflow заново на вкладке
`Actions → Weather bot → Enable workflow`.
